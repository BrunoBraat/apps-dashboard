...
width = st.slider('Escolha o tamanho do gráfico:',
                  min_value=400, max_value=800, value=400)
...
def constroi_grafico_barras(variavel):
    ...
    fig = px.bar(contagem, x=variavel, y='Contagem', text='Contagem',
                 title='Contagem de ' + variavel, labels={'Contagem': 'Quantidade'})
...
def constroi_grafico_hist(variavel):
    ...
    # Criando o gráfico de barras com o Plotly
    fig = px.histogram(df[variavel], nbins=10, title='Histograma de ' +
                       variavel, labels={'value': 'Valor', 'count': 'Contagem'})
...
def constroi_grafico_barras_bivariado(variavel):
    ...
    tabela_contagem = pd.pivot_table(
        df, index=variavel, columns='Funcionario_Deixou_Empresa', aggfunc='size', fill_value=0).reset_index()
...
def constroi_boxplot_bivariado(variavel):
    ...
    fig = px.box(df, x="Funcionario_Deixou_Empresa", y=variavel,
                 title='Impacto de ' + variavel + ' na Rotatividade de Funcionários')
...
pagina_selecionada = st.sidebar.radio(
    'Selecione uma página', ['Análise Univariada', 'Análise Bivariada'])
